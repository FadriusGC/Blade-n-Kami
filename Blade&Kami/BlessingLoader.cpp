...
std::vector<Blessing> BlessingLoader::loadFromFile(
  ...
  while (getline(file, line)) {
    ...
    if (line[0] == '[') {
      ...
      if (!currentId.empty()) {
        blessings.emplace_back(currentId, name, desc, kami, kamiType, type,
                               ability, basePower, reikiCost);
      }
      ...
      type =
          (typeStr == "active") ? BlessingType::ACTIVE : BlessingType::PASSIVE;
  ...
  if (!currentId.empty()) {
    blessings.emplace_back(currentId, name, desc, kami, kamiType, type, ability,
                           basePower, reikiCost);
  }
