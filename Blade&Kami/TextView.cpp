...
void TextView::showEnemyInfo(const Enemy& enemy) {
  std::cout << "\n=== " << enemy.data.name << " ===\n"
            << u8"Здоровье: " << enemy.data.currentHealth << "/"
...
void TextView::showEnemyList(const std::vector<EnemyData>& enemies) {
  ...
  for (const auto& e : enemies) {
    std::cout << "\n[" << e.id << "] " << e.name << u8" (Уровень: " << e.level
              << ")";
  }
  std::cout << "\n";
}
...
void TextView::showCombatStats(const Player& player, const Enemy& enemy) {
  std::cout << u8"\n======= БОЙ =======\n"
            << u8"🥷🏻 Игрок: \n"
            << u8" ├❤️ Здоровье: " << player.currentHealth << u8"/"
            << player.maxHealth << u8"\n"
            << u8" ├🌀 Рэйки: " << player.currentReiki << u8"/"
            << player.maxReiki << "\n"
            << u8" ├💥 Урон: " << player.blade.minDamage << u8"-"
            << player.blade.maxDamage << "\n"
            << u8" ├🎯 Точность: " << (player.blade.accuracy * 100) << u8"%\n"
            << u8" └🍃 Уклонение: " << (player.evasion * 100) << u8"%\n"
            << u8"-------- vs --------\n"
            << u8"👹 Враг (" << enemy.data.name << u8"): \n"
            << u8" ├❤️ Здоровье врага: " << enemy.data.currentHealth << u8"/"
            << enemy.data.maxHealth << u8"\n"
            << u8" ├💥 Урон врага: " << enemy.data.minDamage << u8"-"
            << enemy.data.maxDamage << u8"\n"
            << u8" ├🎯 Точность врага: " << (enemy.data.accuracy * 100)
            << u8"%\n"
            << u8" └🍃 Уклонение врага: " << (enemy.data.evasion * 100)
            << u8"%\n";
}

void TextView::showCombatMenu(Player& player, Enemy& enemy) {
  std::cout << u8"==================\n[1] Атака (Шанс попадания: "
            << (player.blade.accuracy - enemy.data.evasion) * 100 << u8"%)"
            << u8"\n[2] Очищение (Шанс: "
...
void TextView::showAltarMenu(const std::vector<Blessing>& availableBlessings,
  ...
  for (size_t i = 0; i < availableBlessings.size(); ++i) {
    ...
    std::string powerDesc = TextView::generatePowerDescription(
...
void TextView::showBlessingMenu(const std::vector<Blessing>& blessings,
  ...
  if (blessings.empty()) {
    ...
    for (const auto& blessing : blessings) {
      ...
      if (blessing.type == BlessingType::ACTIVE) {
        std::cout << u8"  Рэйки: " << blessing.reikiCost << u8"\n  "
...
void TextView::showBlessingDetails(const Blessing& blessing,
  ...
  std::cout << u8"\n=== ДЕТАЛИ БЛАГОСЛОВЕНИЯ ===\n"
            ...
            << u8"----------------------------\n"
            << u8"Тип: "
            << ((blessing.type == BlessingType::ACTIVE) ? u8"Активное"
                                                        : u8"Пассивное")
            << u8"\n"
            << u8"" << powerDesc << u8"\n";
...
void TextView::showCombatBlessingsMenu(
    const std::vector<Blessing>& activeBlessings, const Player& player) {
...
std::string TextView::generatePowerDescription(const std::string& ability,
  ...
  if (ability == "heal") {
    return u8"Эффект: Восстанавливает " + std::to_string(minPower) + u8"-" +
           std::to_string(maxPower) + u8" здоровья";
  } else if (ability == "damage") {
    return u8"Эффект: Наносит " + std::to_string(minPower) + u8"-" +
           std::to_string(maxPower) + u8" урона";
  } else if (ability == "sun_strike") {
    return u8"Эффект: Наносит " + std::to_string(minPower) + u8"-" +
           std::to_string(maxPower) + u8" урона + 10% от макс. здоровья";
  } else if (ability == "touch_of_death") {
    return u8"Эффект: Наносит " + std::to_string(minPower) + u8"-" +
           std::to_string(maxPower) +
    ...
    return u8"Эффект: Восстанавливает " + std::to_string(minPower) + u8"-" +
           std::to_string(maxPower) + u8" Рэйки";
  ...
  return u8"Сила/Увеличение параметров: " + std::to_string(minPower) + u8"-" +
         std::to_string(maxPower);
}
